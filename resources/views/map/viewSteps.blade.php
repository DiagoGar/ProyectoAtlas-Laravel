...
@section('form')
  <div class="flex flex-col justify-center items-center">
    ...
    <form action="{{ url('/bajarLote') }}" method="GET">
      <label for="patente">Selecciona una patente:</label>
      <select name="patente" id="patente">
          @foreach($data['cc'] as $patente)
              <option value="{{ $patente->id }}">{{ $patente->patente }}</option>
          @endforeach
      </select>
      <button type="submit"><a href="{{ url('/bajarLote')}}">Bajar Lote</a></button>
  </form>
